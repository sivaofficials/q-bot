...
const embedHelp = new Discord.MessageEmbed()
  ...
  .addFields(
    { name: 'Join the queue', value: '`!q join`', inline: false },
    { name: 'Leave the queue', value: '`!q leave`', inline: false },
    { name: 'See how long you have been in the queue', value: '`!q time`', inline: false },
    { name: 'Get a list of available commands', value: '`!q commands` or `!q help`', inline: false },
  )
...
client.on('message', message => {
  ...
  if (!args.length) {
    return message.channel.send(`No commands provided, ${message.author}! Type !q help for a list of queue commands and their usage.`);
  } else {
    subcommand = args[0].toString();
  }

  // command: !queue start
  if (command === 'q' && subcommand === 'start') {
    if (queueMessageId !== 0) {
  ...
  if (command === 'q' && subcommand === 'remove') {
    if (isUserIsAdmin(message.member.roles.cache) && args.length === 2) {
      console.log('User is an admin');
  ...
  // command: !queue help
  if (command === 'q' && (subcommand === 'help' || subcommand === 'commands')) {
